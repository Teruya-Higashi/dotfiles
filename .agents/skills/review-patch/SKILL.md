...
このスキルの実行中は、準備・レビュー・修正・後片付けの全工程で `rm` / `rmdir` を実行しない。一時ファイル・空ディレクトリも対象とし、スクリプトや委譲先経由でも実行しない。不要な一時ファイルやディレクトリは残し、worktree の cleanup は `git worktree remove` を使う。失敗時に `rm` / `rmdir` へフォールバックしない。
