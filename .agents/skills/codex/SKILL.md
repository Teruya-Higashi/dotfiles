...
| effort | `high` | GPT-5.6 Sol: `low`, `medium`, `high`, `xhigh`, `max`, `ultra` | `--effort high`, `--effort max` |
...
- `high`（デフォルト）: 通常の実装・調査を含む委任。速度と推論のバランスを取る
- `xhigh`: 複雑な実装・調査など、より深い推論が必要な場合の明示的な上書き
...
| `--effort {値}` | `-c model_reasoning_effort="{値}"` | config の `model_reasoning_effort` を上書き（未指定時は `high`） |
