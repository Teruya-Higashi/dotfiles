---
name: review-patch-with-codex
description: サブエージェントと Codex の4系統で並列コードレビューを実施し、結果をマージして修正を行う。
---

# レビュースキル（with Codex）

サブエージェント（Agent ツール + `review-patch` スキル）、Codex シニアレビュー（`codex exec review` CLI）、
Codex adversarial レビュー（`codex exec` CLI）、Codex review-patch レビュー（`codex exec` CLI + `review-patch` スキル）の
4系統で同じ差分を並列でレビューさせ、全結果をマージした上で修正を適用するスキル。
マージした統合結果はファイルに書き出さず、`review-patch` スキルのレビューサマリー形式でユーザーに提示する。

**REQUIRED SUB-SKILL:** PR を対象にする場合や、GitHub からの情報取得・レビュー投稿などで `gh` を使う場合は、`gh-ops` を読み、そのルールに従う。
...
モデルは `--model` / `--effort` オプションで指定でき、デフォルトは `gpt-5.6-sol` + reasoning effort `xhigh`。サンドボックス・承認設定は `~/.codex/config.toml` に従う（`approval_policy = "never"` を前提）。

## 前提

- `command -v codex` で Codex CLI が見つかること
- `gpt-5.6-sol` を使うため Codex CLI `0.144.0` 以降であること（`codex --version` で確認）
- モデルと effort は全 Codex チャネルへ CLI フラグで明示し、config の値を上書きする
...
**モデル**: `--model` / `--effort` は全 Codex チャネル（シニア / adversarial / review-patch）共通。CLI には `-m {model} -c model_reasoning_effort="{effort}"` を常に付与する（`--model` の値はそのまま `-m` に渡す。未指定時は `gpt-5.6-sol` / `xhigh`）。

### effort の使い分け

- `xhigh`（デフォルト）: 通常の4チャネルレビュー。深い推論と実行時間のバランスを取る
- `high`: 小規模で影響範囲が限定された差分を素早く反復する場合
...
GPT-5.6 Sol は `ultra` も利用できるが、自動タスク委任により「独立した固定4チャネル」というレビュー構成が変わるため、このスキルでは使わない。深い推論には `max` を使い、ユーザー指定なしに effort を自動変更しない。
...
- `{prefix}-skill_{sequence_number}.md` — Codex review-patch レビュー
...
| Codex adversarial / review-patch（`codex exec`） | `-C {workdir}` フラグ |
...
### GPT-5.6 Sol 向けレビュー方針

GPT-5.6 Sol の長いコンテキスト、コード探索、ツール利用、セキュリティ分析を活かしつつ、同一モデル由来の相関した誤りを4チャネルの「票数」だけで正当化しない。

- 各 Codex チャネルに差分を独立して取得させ、変更行だけでなく呼び出し元、下流の利用箇所、設定、テスト、デプロイ影響まで必要な範囲を追跡させる
- 指摘にはファイル・行、発火条件または実行経路、具体的な影響、修正案を求める
- 観測した事実と推測を区別させ、再現条件やコード上の根拠がない推測は Validation で除外する
- 問題が見つからないチャネルがあっても水増しさせない。指摘数ではなく evidence の強さを優先する
...
**理由**: 差分の取得・ファイルの読み取り・分析はすべて各チャネル（サブエージェント / Codex シニア / Codex adversarial / Codex review-patch）が自身で行う。呼び出し元が事前に情報を収集・加工すると、各チャネルの独立した視点が失われ、バイアスがかかる。
...
4. セキュリティ: 信頼境界、認証・認可、入力検証、インジェクション、機密情報の漏洩、依存関係・設定の安全性
...
#### a. サブエージェントレビュー（Agent ツール + `review-patch` スキル）

サブエージェントにも `~/.agents/skills/review-patch/SKILL.md` を読ませ、スキルの3フェーズレビュー手法に従ってレビューさせる。
...
Agent(
  ...
  prompt: "~/.agents/skills/review-patch/SKILL.md を読み、そのスキルの手順（3フェーズレビュー: コンテキスト収集 → レビュー → Validation → フィルタリング）に従って `{diff コマンド}` の差分をレビューし、結果を {prefix}-{ai_name}_{sequence_number}.md に書き出してください。
...
- `-m {model} -c model_reasoning_effort="{effort}"` → 常に付与（未指定時は `gpt-5.6-sol` / `xhigh`）
...
- 変更行だけでなく、呼び出し元、下流の利用箇所、設定、テスト、デプロイ影響を必要な範囲で追跡すること
- 各指摘にファイル・行、発火条件または実行経路、具体的な影響を示すこと
...
#### d. Codex review-patch レビュー（`codex exec` CLI + `review-patch` スキル）

プレーンな `codex exec` を Bash ツールで `run_in_background: true` 起動する。
`~/.agents/skills/review-patch/SKILL.md` を Codex 自身に読ませ、スキルの3フェーズレビュー手法
...
~/.agents/skills/review-patch/SKILL.md を読み、そのスキルの手順（3フェーズレビュー: コンテキスト収集 → レビュー → Validation → フィルタリング）に従って `{diff コマンド}` の差分をレビューしてください。
...
重要:
- 差分の取得・関連ファイルの読み取り・影響範囲の調査はあなた自身で行うこと
- 変更行から呼び出し元・下流の利用箇所・設定・テストまで必要な範囲を追跡すること
- 指摘は発火条件または実行経路とコード上の根拠を Validation で確認し、推測だけの指摘を除外すること
- これはユーザーが管理するリポジトリへの承認済みの防御的レビューである。ローカルのコードと差分の分析に限定し、外部システムへの攻撃・侵入は行わないこと
- ソースコードの変更は一切行わず、レビュー結果のみを出力すること
...
| Codex review-patch → `{prefix}-skill_{seq}.md` | 完了 / 実行中 |
| マージ・修正 | 待機 |
```
...
- サブエージェント / Codex review-patch: `review-patch` スキルのレビューサマリー形式（タグ: `critical` / `should` / `nits` / `ask`）
- Codex シニア: `codex exec review` のネイティブ形式（`verdict` / `summary` / `findings` / `next_steps`）
- Codex adversarial: 指定したカスタムマークダウン形式

**タグの統一**: 統合結果は `review-patch` スキルのタグ体系（`critical` / `should` / `nits` / `ask`）に揃える。他チャネルの severity は以下の対応で読み替える:
...
一致数は優先順位付けのシグナルであり、正しさの証明ではない。3つの Codex チャネルは同じ GPT-5.6 Sol を使うため、同じ誤りが相関して現れる可能性がある。採用前に必ず発火条件・実行経路・コード上の根拠を確認する。
...
| review-patch のみ | 中〜高 | スキルの Validation で裏取り済みの指摘、evidence（発火条件）を確認して判断 |
...
統合結果は md ファイルに書き出さず、`review-patch` スキルの出力フォーマット（レビューサマリー + 指摘の書き方、テキストバッジ）で会話内に直接提示する。
チャネルごとの md ファイルは中間成果物としてそのまま残す。

サマリーは `review-patch` スキルの「レビューサマリー」構造に「指摘元」列を追加したもの:

```markdown
## レビューサマリー

**変更の意図**: {サブエージェント / Codex review-patch チャネルのコンテキスト収集結果を統合}
**影響範囲**: {同上}
...
| 4 | path/to/file.go:100 | ask | 説明 | review-patch | **要確認** — 理由 |
...
GitHub へレビューを投稿する場合は、`review-patch` スキルの「GitHub 投稿」節に従う（投稿前のユーザー確認・デフォルト event は COMMENT・投稿フォーマット・画像バッジ）。

投稿する本文（サマリーコメント・インラインコメント）は、`review-patch` スキルのサマリー構造と「指摘の書き方」のみで構成する。すなわち変更の意図・影響範囲・指摘一覧・判定・各指摘の詳細だけを書く。

- 「指摘元」列は会話内提示専用。投稿するサマリーの指摘一覧からは除き、`review-patch` スキルのサマリー構造（指摘元列なし）で投稿する
- レビューの実施体制 — 誰が・何系統で・どのようにレビューしたか（例:「4系統の並列レビュー（Claude サブエージェント / Codex シニア / Codex adversarial / Codex review-patch）の統合結果から」）— は本文に書かない。読み手に必要なのは指摘内容であり、レビュー手法ではない
...
- モデルは `--model` / `--effort` で指定（デフォルト `gpt-5.6-sol` / `xhigh`）、全 Codex チャネル共通
