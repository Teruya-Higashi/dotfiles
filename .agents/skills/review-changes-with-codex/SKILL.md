...
**モデル**: `--model` / `--effort` は全 Codex チャネル（シニア / adversarial / review-changes）共通。CLI には `-m {model} -c model_reasoning_effort="{effort}"` を常に付与する（`--model` の値はそのまま `-m` に渡す。未指定時は `gpt-5.6-sol` / `xhigh`）。
...
```text
/review-changes-with-codex --effort high --target staged
/review-changes-with-codex --effort max --target pr
```
